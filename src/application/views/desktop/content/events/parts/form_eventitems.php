...
?>
			...
			<div> 
				<?php if ($eventItem->{DB_EVENTITEM_MAXPCS} == 0) { ?>
					...
					<?php } ?>																															
				<?php } else { ?>
					<select name="<?php echo $fieldPrefix . $eventItem->{DB_EVENTITEM_ID}; ?>_<?php echo DB_PERSONHASEVENTITEM_AMOUNT; ?>" class="short" <?php echo (!is_null($eventItem->{DB_EVENTITEM_AMOUNT}) ? 'data-price="' . $eventItem->{DB_EVENTITEM_AMOUNT} . '"' : ''); ?>>
